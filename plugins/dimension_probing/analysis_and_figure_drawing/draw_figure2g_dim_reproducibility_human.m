...
refdir = fullfile(data_dir,'reference_models_human');
for i_model = 1:20
...
fig = figure('Position',[800 800 624 500],'color','none');
hold on
x = [1:dim dim:-1:1];
...
plot(reproducibility,'o','MarkerFaceColor',[.2 .4 .6],'MarkerEdgeColor','none','MarkerSize',3)
ylim([0 1])
xlim([0 dim+1])
set(gca,'FontSize',17) 
ylabel('Reproducibility score', 'FontSize', 21);
xlabel('Human embedding dimension number', 'FontSize', 21);
hax = gca;
hax.LineWidth = 1.5; 
if dosave
    exportgraphics(fig, 'dim_reproducibility_human.pdf', 'ContentType', 'vector');
end
