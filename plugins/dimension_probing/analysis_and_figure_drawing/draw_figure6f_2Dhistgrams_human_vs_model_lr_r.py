...
for subj in subject_list:
    ...
    mat_file_path = 'data/ROIs/Subject_' + str(subj) + '/fmri_shared_1k_all_rois_3d_r_gemini.mat'
    mat_contents = loadmat(mat_file_path)
    test_data = mat_contents['data']
    flattened_pcc_mllm = test_data.ravel()

    mat_file_path = 'data/ROIs/Subject_' + str(subj) + '/fmri_shared_1k_all_rois_3d_r_chatgpt.mat'
    mat_contents = loadmat(mat_file_path)
    test_data = mat_contents['data']
    flattened_pcc_llm = test_data.ravel()

    mat_file_path = 'data/ROIs/Subject_' + str(subj) + '/fmri_shared_1k_all_rois_3d_r_humans.mat'
    mat_contents = loadmat(mat_file_path)
    test_data = mat_contents['data']
    ...
    plt.savefig('figures/2dhist_llm_vs_human_'+str(subj)+'_lr_r.pdf', bbox_inches='tight', pad_inches=0)
    ...
    ax.spines['left'].set_linewidth(3)
    ax.spines['right'].set_linewidth(3)
    plt.savefig('figures/2dhist_mllm_vs_human_'+str(subj)+'_lr_r.pdf', bbox_inches='tight', pad_inches=0)