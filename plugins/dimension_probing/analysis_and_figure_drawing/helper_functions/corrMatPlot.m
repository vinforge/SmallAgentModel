classdef corrMatPlot
    ...
    properties
        ax;drawn=false;
        arginList={'XLabels','YLabels','colorMap','Format','Type'}
        %         Labels={};
        XLabels={};
        YLabels={};
        colorMap={};
    ...
    methods
        function obj=corrMatPlot(varargin)
            ...
            if isempty(obj.XLabels)
                for i=1:obj.num
                    obj.XLabels{i}=['class ',num2str(i)];
                end
            end

            if isempty(obj.YLabels)
                for i=1:obj.num
                    obj.YLabels{i}=['class ',num2str(i)];
                end
            end
            help corrMatPlot
        end
        ...
        function obj=draw(obj)
            ...
            % 绘制colorbar 
%             cb.Limits=[-1.02,1.02];
            cb.Limits=[min(obj.corrMat(:)) - 0.02, max(obj.corrMat(:)) + 0.02];
            cb.TickDirection='out';
            cb.LineWidth=.8;
%             cb.Ticks=-1:.2:1;
            cb.Ticks=round(min(obj.corrMat(:)) : .1 : max(obj.corrMat(:)), 2);
            cb.FontName='Arial';
            cb.FontSize=11;
            ...
            for row=1:obj.num
                obj.RLabelHdl(row)=text(.42+(row-1).*strcmp(obj.Format,'triu'),obj.num+1-row,...
                    obj.YLabels{row},'HorizontalAlignment','right',...
                    'FontName','Arial','FontSize',12);
            end
            for col=1:obj.num
                obj.CLabelHdl(col)=text(col-.1,obj.num+.75-(col-1).*strcmp(obj.Format,'tril'),...
                    obj.XLabels{col},'HorizontalAlignment','left',...
                    'FontName','Arial','FontSize',12,'Rotation',70);
            end
            obj.drawn=true;
            % -------------------------------------------------------------
            % 渐变色句柄生成函数
        ...
        % XY轴标签设置
        function obj=setXLabelStr(obj,LB)
            obj.XLabels=LB;
            if obj.drawn
                for i=1:obj.num
                    set(obj.CLabelHdl(i),'String',obj.XLabels{i});
                end
            end
        end

        function obj=setYLabelStr(obj,LB)
            obj.YLabels=LB;
            if obj.drawn
                for i=1:obj.num
                    set(obj.RLabelHdl(i),'String',obj.YLabels{i});
                end
            end
        end
