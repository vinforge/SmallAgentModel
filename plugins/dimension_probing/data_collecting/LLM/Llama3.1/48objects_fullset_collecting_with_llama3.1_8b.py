...
openai_api_key = '0338f2e7-a5cd-4859-aa50-c9e0ea121b89'
model_engine = "Meta-Llama-3.1-8B-Instruct"
post_link = 'https://api.sambanova.ai/v1/chat/completions'
temperature = 0.01 # The larger the value, the more random the response
max_tokens = 1024
n = 1
count = 0

base_path = 'C:/D/数据集/NMI2024_code&data/analysis_and_figure_drawing/'

filename = base_path +'data/LLMs/Llama3.1/triplet_dataset/full_sampling_for_48_objects_Llama3.1_8b.txt'
if not os.path.exists(filename):
    with open(filename, 'w') as file:
        pass
line_count = count_lines(filename)

full_sampling = base_path +"data/LLMs/ChatGPT-3.5/triplet_dataset/full_sampling_for_48_objects_ChatGPT-3.5(without_overlaping_with_trainset).txt"
total_lines = count_lines(full_sampling)

# Read image descriptions and numbers from a txt file, which was saved in advance.
with open(base_path +'data/LLMs/ChatGPT-3.5/image_descriptions_meta.txt', 'r') as f:
...
for trial in range(start_trial,end_trial):
    ...
    write_trial_data_to_csv(base_path +'data/LLMs/Llama3.1/triplet_dataset/prompts&responses_full_sampling_for_48_objects_Llama3.1_8b.csv', group_results, group_prompts, group_responses)

    print("model:", sorted_images)
    # Save the sorted image numbers to a txt file
    with open(base_path +'data/LLMs/Llama3.1/triplet_dataset/full_sampling_for_48_objects_Llama3.1_8b.txt', 'a') as f:
